...
CREATE TABLE IF NOT EXISTS `balloonloop_db.user_property` (
  ...
  install_at            DATETIME OPTIONS(description='설치 시각 (UTC). 90일 정책 기준값'),
  ...
  last_active_at        DATETIME OPTIONS(description='마지막 앱 실행 시각 (UTC)'),
  last_active_version   STRING   OPTIONS(description='마지막 활성 앱 버전'),
  last_active_country   STRING   OPTIONS(description='마지막 활성 국가'),
  last_played_at        DATETIME OPTIONS(description='마지막 레벨 플레이 시각 (UTC). churn 판정 기준'),
  ...
  infinite_lives_expiry DATETIME OPTIONS(description='무한 하트 만료 시각 (UTC). NULL=비활성'),
  is_payer              BOOL     OPTIONS(description='결제자 라벨 — verified 1건 이상, 영구 TRUE'),
  last_updated_at       DATETIME OPTIONS(description='마지막 갱신 (UTC). 세션 시작/종료 UPSERT'),
