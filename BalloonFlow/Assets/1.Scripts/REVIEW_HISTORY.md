...
## Phase 16: 디자인 문서 동기화 (2026-03-15)

### 배경
신규 디자인 문서(BeatChart_Direction, 기믹명세, 아웃게임디렉션, 피드백디렉션)와 기존 코드 사이의 불일치 확인 후 코드 수정.

### 16-1: 부스터 시스템 재설계 (BoosterManager.cs, ShopManager.cs, DailyRewardManager.cs)
- **변경**: 6개 부스터(Pre/In + 젬 혼합) → 4개 인플레이 부스터(전부 코인)
- **새 부스터**: Extra Tray(300), Select Tool(1900), Shuffle(1500), Color Remove(2900)
- **해금 레벨**: Lv.10, Lv.12, Lv.15, Lv.18 (디자인 반영)
- **젬 의존성 제거**: 모든 부스터 코인 결제로 통일
- **ShopManager**: 상품 목록 4종으로 갱신, 번들 보상 상수 변경
- **DailyRewardManager**: 상수를 BoosterManager 참조로 변경

### 16-2: 이어하기 첫 무료 (ContinueHandler.cs)
- **변경**: 젬 기반 에스컬레이션(30/40/50/60 gem) → 코인 기반 + 첫 무료
- **새 비용**: FREE → 900 → 1900 → 2900 코인 (총 4회, 1무료+3유료)
- **리스타트 시 리셋**: 비용이 다시 무료로 복귀

### 16-3: 초기 코인 변경 (CurrencyManager.cs)
- **변경**: 2000 → 1000 (디자인: 약 1회 이어하기 세션 분량)

### 16-4: 스타 보상 조정 (CurrencyManager.cs)
- **변경**: 1★=100, 2★=200, 3★=300 → 1★=50(Normal), 2★=75(Hard), 3★=100(SuperHard)
- **CoinSink enum**: BoosterTrayAdd/BoosterHand → BoosterExtraTray/BoosterSelectTool

### 16-5: 젬 UI 비활성화 (GameBootstrap.cs)
- **변경**: v1.0은 코인 전용, 젬은 소프트런칭 후 추가
- GemButton/GemDisplayText 생성 제거
- OnGemChanged 이벤트 구독 제거
- GemManager 싱글톤 생성 제거 (파일은 유지)
- 상점 헤더 젬 표시 제거

### 16-6: 홀더 경고 피드백 추가 (P0)
- **GameEvents.cs**: `OnHolderWarning` 이벤트 구조체 추가 (waitingCount, maxSlots, isDanger)
- **HolderManager.cs**: `CheckAndPublishWarning()` 추가 — 4/5(warning), 5/5(danger) 발행
- **FeedbackController.cs**: `HandleHolderWarning()` 추가 — 경고/위험 시 화면 흔들림 + SFX
- 새 AudioClip 필드: `_holderWarningClip`, `_holderDangerClip`

### 수정 파일 목록

| 파일 | 항목 | 변경 내용 |
|------|------|-----------|
| `BoosterManager.cs` | 16-1 | 4종 코인부스터, 해금레벨, 젬 제거 |
| `ContinueHandler.cs` | 16-2 | 코인기반 + 첫무료, 4회 최대 |
| `CurrencyManager.cs` | 16-3,4 | 초기 1000, 보상 50/75/100, CoinSink 갱신 |
| `GameBootstrap.cs` | 16-5 | 젬 UI 비활성화, 초기값 1000 |
| `ShopManager.cs` | 16-1 | 부스터 상품 4종 갱신, 번들 상수 |
| `DailyRewardManager.cs` | 16-1 | 부스터 상수 참조 변경 |
| `GameEvents.cs` | 16-6 | OnHolderWarning 이벤트 추가 |
| `HolderManager.cs` | 16-6 | CheckAndPublishWarning 추가 |
| `FeedbackController.cs` | 16-6 | 홀더 경고/위험 피드백 핸들러 |
